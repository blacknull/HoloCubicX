...
MjpegPlayDocoder::MjpegPlayDocoder(File *file, bool isUseDMA)
{
    ...
    log_i("in video_start()");
    video_start();
    log_i("out video_start()");
}

MjpegPlayDocoder::~MjpegPlayDocoder(void)
{
    log_i("~MjpegPlayDocoder");
...
bool MjpegPlayDocoder::video_start()
{
    ...
    // log_i("heap_caps_get_free_size(): ");
    // log_i((unsigned long)heap_caps_get_free_size(MALLOC_CAP_8BIT));
    // log_i("heap_caps_get_largest_free_block() :");
    // log_i((unsigned long)heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));

    // log_i("heap_caps_dump_all() :");
    // heap_caps_dump_all();

    // log_i("ESP.getFreeHeap()--------->");
    // log_i(ESP.getFreeHeap());
